const addresses = require("../addresses/projectOneAbi"); // Get all relevant Ethereum and BSC addresses
...
const getProjectOneData = async (web3s) => {
  ...
  let bsc_projectOne;
  try {
    bsc_projectOne = new bsc_web3.eth.Contract(
      projectOneAbi,
      bsc_addresses.contract
    );
  } catch (err) {
  ...
  const burnt_on_eth = await projectOne.methods
    .balanceOf(eth_addresses.burnt)
    .call();

  tokenData.eth.totalSupply.value = await projectOne.methods
    .totalSupply()
    .call();
  const team_1 = await projectOne.methods
    .balanceOf(eth_addresses.team_1)
    .call();
  const team_2 = await projectOne.methods
    .balanceOf(eth_addresses.team_2)
    .call();
  const team_3 = await projectOne.methods
    .balanceOf(eth_addresses.team_3)
    .call();

  // Get base BSC values
  try {
    burnt_on_bsc = await bsc_projectOne.methods
      .balanceOf(bsc_addresses.burnt)
      .call();
  } catch (err) {
  ...
  try {
    tokenData.bsc.totalSupply.value = await bsc_projectOne.methods
      .totalSupply()
      .call();
  } catch (err) {
  ...
  let bsc_team_1;
  try {
    bsc_team_1 = await bsc_projectOne.methods
      .balanceOf(bsc_addresses.team_1)
      .call();
  } catch (err) {
  ...
  let bsc_team_2;
  try {
    bsc_team_2 = await bsc_projectOne.methods
      .balanceOf(bsc_addresses.team_2)
      .call();
  } catch (err) {
  ...
  tokenData.eth.circulatingSupply.value =
    Number(tokenData.eth.totalSupply.value) - Number(team_eth);

  // Get derived values BSC
  const team_bsc = Number(bsc_team_1) + Number(bsc_team_2);
  tokenData.bsc.totalSupply.value -= burnt_on_bsc;
  tokenData.bsc.circulatingSupply.value =
    Number(tokenData.bsc.totalSupply.value) - Number(team_bsc);

  // Get joint values
  tokenData.combined.totalSupply.value =
    tokenData.bsc.totalSupply.value + tokenData.eth.totalSupply.value;
  tokenData.combined.circulatingSupply.value =
    Number(tokenData.bsc.circulatingSupply.value) +
    Number(tokenData.eth.circulatingSupply.value);
  ...
  tokenData.eth.circulatingSupply.description =
    "Circulating supply of projectOne on ETH";
  tokenData.bsc.circulatingSupply.description =
    "Circulating supply of projectOne on BSC";

  tokenData.combined.totalSupply.description =
    "Total supply of projectOne (BSC & ETH)";
  tokenData.combined.circulatingSupply.description =
    "Circulating supply of projectOne (BSC & ETH)";
  ...
  tokenData.eth.circulatingSupply.name =
    "Circulating Supply of projectOne on ETH";
  tokenData.bsc.circulatingSupply.name =
    "Circulating Supply of projectOne on BSC";

  tokenData.combined.totalSupply.name =
    "Total Supply of projectOne on (BSC & ETH)";
  tokenData.combined.circulatingSupply.name =
    "Circulating Supply of projectOne on (BSC & ETH)";
  ...
  Object.keys(tokendata_combined).forEach((key) => {
    ...
    tokendata_combined[key].formattedValue = numeral(
      tokendata_combined[key].value
    ).format();
  ...
  Object.keys(tokendata_eth).forEach((key) => {
    ...
    tokendata_eth[key].formattedValue = numeral(
      tokendata_eth[key].value
    ).format();
  ...
  Object.keys(tokendata_bsc).forEach((key) => {
    ...
    tokendata_bsc[key].formattedValue = numeral(
      tokendata_bsc[key].value
    ).format();
